...
class SimpleSetupModal(ui.Modal, title="🔧 Minecraft Server Setup"):
    ...
    ram = ui.TextInput(
        ...
        placeholder="Enter number only, e.g., 4",
        default="4",
        max_length=2,
        required=False
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # Parse max_players - strip non-numeric characters
            try:
                max_players_str = (self.max_players.value or "20").strip()
                # Remove any non-digit characters (like 's', 'g', etc.)
                max_players_clean = ''.join(filter(str.isdigit, max_players_str))
                if not max_players_clean:
                    raise ValueError("No number provided")
                max_players = int(max_players_clean)
                if not 1 <= max_players <= 100:
                    raise ValueError("Out of range")
            except ValueError as e:
                await interaction.response.send_message(
                    f"❌ Max players must be a **number only** between 1 and 100.\n"
                    f"You entered: `{self.max_players.value}`\n"
                    f"Example: `20` (not `20s` or `20 players`)", 
                    ephemeral=True
                )
                return
            
            # Parse RAM - strip non-numeric characters
            try:
                ram_str = (self.ram.value or "4").strip()
                # Remove any non-digit characters (like 's', 'g', 'gb', etc.)
                ram_clean = ''.join(filter(str.isdigit, ram_str))
                if not ram_clean:
                    raise ValueError("No number provided")
                ram = int(ram_clean)
                if not 1 <= ram <= 32:
                ...
                await interaction.response.send_message(
                    f"❌ RAM must be a **number only** between 1 and 32.\n"
                    f"You entered: `{self.ram.value}`\n"
                    f"Example: `4` (not `4s` or `4GB`)", 
                    ephemeral=True
                )
                return
