...
3. Link found, `is_premium=False` (cracked account):
   - Within 30-minute grace window (tracked in `recently_disconnected`) → allow.
   - Outside grace window → kick + DM Discord user a 6-char alphanumeric Challenge code.
4. `handle_player_quit(username)` starts the 30-minute grace timer.
...
- `CodeModal(discord.ui.Modal)` — 6-character `TextInput` for code submission. On correct code: calls `join_guard.complete_challenge()`.
