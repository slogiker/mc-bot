...
├── docker-compose.yml          # mc-bot + playit services
...
│   ├── ai.py                   # /ai — Grok chat integration
...
│   ├── economy.py              # /balance, /pay, /economy_set + Word Hunt
...
| `PLAYIT_SECRET_KEY` | ✅       | Playit.gg agent secret key                  |
...
- `crash_check` (every 30s): If server not running and not intentionally stopped → attempt restart → notify debug channel
