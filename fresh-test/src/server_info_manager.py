...
class ServerInfoManager:
    ...
    async def get_or_create_channel(self, guild: discord.Guild) -> discord.TextChannel:
        ...
        channel_id = config.get('server_info_channel_id')
        if channel_id:
        ...
        with config.update_bot_config() as bot_config:
            bot_config['server_info_channel_id'] = channel.id
    ...
    async def _get_seed(self) -> str:
        ...
        cached = bot_cfg.get('cached_seed')
        if cached:
            return cached
        ...
        if seed == 'Unknown':
            try:
                import nbtlib
                level_dat = os.path.join(config.SERVER_DIR, getattr(config, 'WORLD_FOLDER', 'world'), "level.dat")
                if os.path.exists(level_dat):
                    data = await asyncio.to_thread(nbtlib.load, level_dat)
                    # 1.18+ format
                    try:
                        seed = str(int(data["Data"]["WorldGenSettings"]["seed"]))
                    except (KeyError, TypeError):
                        pass
                    # Pre-1.18 format
                    if seed == 'Unknown':
                        try:
                            seed = str(int(data["Data"]["RandomSeed"]))
                        except (KeyError, TypeError):
                            pass
            except Exception as e:
