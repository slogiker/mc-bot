...
class MinecraftInstaller:
    ...
    def __init__(self):
        ...
        # Dynamic Java Path - default to system 'java'
        self.java_path = "java"

    async def get_required_java_version(self, version: str) -> int:
        """Determine required Java version for a given MC version"""
        try:
            # 26.x (2026 releases) require Java 25+
            if version.startswith("26."):
                return 25
            
            # Handle standard semantic versioning
            v_parts = [int(p) for p in version.split('.')]
            if v_parts[1] >= 20 and (len(v_parts) < 3 or (len(v_parts) >= 3 and v_parts[2] >= 5)): # 1.20.5+
                return 21
            if v_parts[1] >= 18: # 1.18 - 1.20.4
                return 17
            if v_parts[1] >= 17: # 1.17
                return 16
        except Exception:
            pass
        return 8 # Fallback for old versions

    async def ensure_java(self, mc_version: str):
        """Ensure the correct Java version is available"""
        required_v = await self.get_required_java_version(mc_version)
        
        # Check if we already have it in /app/data/jdk{V}
        local_jdk = os.path.join("/app/data", f"jdk{required_v}")
        java_bin = os.path.join(local_jdk, "bin", "java")
        
        if os.path.exists(java_bin):
            self.java_path = java_bin
            config.JAVA_PATH = java_bin
            return True

        if required_v <= 21: # Assume system Java 21 is okay for <= 21
            self.java_path = "java"
            config.JAVA_PATH = "java"
            return True

        # Need to download JDK (e.g. for Java 25)
        logger.info(f"Downloading required JDK {required_v} for Minecraft {mc_version}...")
        try:
            jdk_tar = os.path.join("/app/data", f"jdk{required_v}.tar.gz")
            url = f"https://api.adoptium.net/v3/binary/latest/{required_v}/ga/linux/x64/jdk/hotspot/normal/eclipse"
            
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as resp:
                    if resp.status != 200:
                        logger.error(f"Failed to download JDK {required_v}: HTTP {resp.status}")
                        return False
                    async with aiofiles.open(jdk_tar, 'wb') as f:
                        await f.write(await resp.read())
            
            os.makedirs(local_jdk, exist_ok=True)
            import subprocess
            subprocess.run(["tar", "-xzf", jdk_tar, "-C", local_jdk, "--strip-components=1"], check=True)
            
            self.java_path = java_bin
            config.JAVA_PATH = java_bin
            with config.update_bot_config() as data:
                data['java_path'] = java_bin
            logger.info(f"JDK {required_v} installed at {java_bin}")
            return True
        except Exception as e:
            logger.error(f"Failed to setup JDK {required_v}: {e}")
            return False
    ...
    async def download_server(self, platform: str, version: str, progress_callback=None) -> tuple[bool, str]:
        ...
        """
        try:
            # Ensure correct Java version for this MC version
            if progress_callback:
                await progress_callback("🔍 Checking Java requirements...")
            await self.ensure_java(version)
