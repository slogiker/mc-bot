...
class Tasks(commands.Cog):
    ...
    async def crash_check(self):
        ...
        try:
            ...
            if playit_secret:
                proc = await asyncio.create_subprocess_exec("tmux", "has-session", "-t", "playit", stderr=asyncio.subprocess.DEVNULL)
                await proc.wait()
                ...
                if not playit_running:
                    ...
                    verify_proc = await asyncio.create_subprocess_exec("tmux", "has-session", "-t", "playit", stderr=asyncio.subprocess.DEVNULL)
