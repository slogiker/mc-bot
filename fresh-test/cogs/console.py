...
class ConsoleCog(commands.Cog):
    ...
    async def cog_load(self):
        """Start the log relay task when the cog is loaded."""
        self._relay_running = True
        self._relay_task = asyncio.create_task(self._log_relay_loop())
        logger.info("ConsoleCog: Log relay task started")

    async def cog_unload(self):
        """Stop the log relay task when the cog is unloaded."""
        self._relay_running = False
        if self._relay_task:
            self._relay_task.cancel()
        logger.info("ConsoleCog: Log relay task stopped")

    async def _log_relay_loop(self):
        """Background task to relay logs from dispatcher to Discord channels in batches."""
        from src.log_dispatcher import log_dispatcher
        queue = log_dispatcher.subscribe()
        ...
        try:
            while self._relay_running:
                try:
                    # Wait for a line, but wake up periodically to flush the buffer
                    line = await asyncio.wait_for(queue.get(), timeout=1.0)
                    
                    if "RCON Client" not in line and "issued server command: /list" not in line:
                        log_buffer.append(line)
                        
                    # Send to #debug ONLY if it's a critical error (no warnings)
                    if any(x in line.upper() for x in ["ERROR", "EXCEPTION", "FAILED TO BIND", "CRASH"]):
                        debug_ch = self.bot.get_channel(config.DEBUG_CHANNEL_ID)
                        if debug_ch:
                            try:
                                await debug_ch.send(f"⚠️ **Server Error Alert:**\n```log\n{line[:1900]}\n```")
                            except Exception: pass
                            
                except asyncio.TimeoutError:
                    pass # Just time to check if we should flush
                    
                current_time = asyncio.get_event_loop().time()
                
                # Flush buffer if interval passed OR buffer is getting large
                if log_buffer and (current_time - last_send_time > batch_interval or len(log_buffer) > 20):
                    log_ch = self.bot.get_channel(config.LOG_CHANNEL_ID)
                    if log_ch:
                        # Combine lines, chunk into 1900 char blocks to respect Discord limits
                        full_text = "\n".join(log_buffer)
            ...
            logger.error(f"ConsoleCog: Error in log relay loop: {e}")
        finally:
            log_dispatcher.unsubscribe(queue)
