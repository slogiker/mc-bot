...
class ControlPanelView(ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        button_id = interaction.data.get('custom_id', 'Unknown')
        logger.info(f"Control Panel Button '{button_id}' clicked by user {interaction.user.name} ({interaction.user.id})")
        return True
        ...
        try:
            ...
            if self.bot.server.is_running():
                ...
                try:
                    ...
                    match = re.search(r"There are (\d+) of a max of (\d+) players online:(.*)", str(players_response))
                    if match:
                        count_str, max_str, names_str = match.groups()
                        if count_str == "0":
                            players_formatted = f"There are 0/{max_str} players."
                        else:
                            names = [n.strip() for n in names_str.split(",") if n.strip()]
                            players_formatted = f"There are {count_str}/{max_str} players:\n" + "\n".join([f"👤 {n}" for n in names])
...
class ControlPanelCog(commands.Cog):
    ...
    async def update_panel(self, silent=True):
        ...
        if not os.path.exists(server_jar_path):
            ...
            is_online = self.bot.server.is_running()
            ...
            if is_online:
                try:
                    success, players_response = await rcon_cmd("list")
                    if success:
                        # Extract "0/20" from "There are 0 of a max of 20 players online: "
                        match = re.search(r'(\d+)\s+of\s+a\s+max\s+of\s+(\d+)', players_response)
                        if match:
                            embed.add_field(name="Players", value=f"`{match.group(1)}/{match.group(2)}`")
                except Exception:
                    pass
        ...
        if self.message_id:
            try:
                ...
                self.message_id = None
                with config.update_bot_config() as bot_config:
        ...
        # Let's delete previous messages from bot in this channel to keep it clean
        try:
            async for old_msg in channel.history(limit=50):
                if old_msg.author == self.bot.user and old_msg.id != self.message_id:
                    if old_msg.embeds and "Control Panel" in str(old_msg.embeds[0].title):
                        await old_msg.delete()
        except discord.HTTPException: pass

        try:
            # Send with silent=True if supported by the library version, or just send
            new_msg = await channel.send(embed=embed, view=view, silent=silent)
