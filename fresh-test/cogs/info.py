...
class Info(commands.Cog):
    ...
    async def _get_tps_info(self) -> str:
        ...
        Attempts Paper/Forge direct TPS, then modern Vanilla /tick query, then legacy debug.
        """
        tps = "Unknown"
        try:
            ...
            # Attempt modern Vanilla (1.20.3+) Tick Query
            try:
                 success_tick, tick_raw = await rcon_cmd("tick query")
                 if success_tick and "Target tick rate:" in tick_raw:
                     match = re.search(r"(?i)current tps:\s*([\d.]+)", tick_raw)
                     if match:
    ...
    async def info(self, interaction: discord.Interaction):
        ...
        try:
            ...
            try:
                ...
                if playit_cog:
                    if not playit_cog._is_cache_valid() or not playit_cog.tunnels:
                        await playit_cog.fetch_playit_address()
